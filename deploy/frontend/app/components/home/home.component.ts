...
export class HomeComponent implements OnInit {
  ...
  loadCharts() {
    ...
    this.http.get<any>(`${this.apiUrl}/stats/duration/year`, { headers })
      .subscribe((statsYear: any) => {
        ...
        this.createChart('chartYear', statsYear, "Année ");
      });
  }
  ...
  createChart(canvasId: string, stats: any, title: string) {
    ...
    const data = {
      labels: [ 'تتم معالجتها', 'المراسلات في إنتظار جواب السلطات', 'عدد المراسلات ردت عليها السلطات', 'مغلقة'],
      datasets: [{
        data: [ stats.enCours, stats.envoyeesAutorite, stats.reponduAutorite, stats.cloturees],
