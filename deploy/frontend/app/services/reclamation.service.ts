...
export class ReclamationService {
  ...
  getReclamationsPourRetourAvecDetails(): Observable<Reclamation[]> {
    
    const headers = this.getAuthHeaders();

    return this.http.get<Reclamation[]>(`${this.apiUrl}/retourAutorite/details`, { headers });
  }
  ...
  retourAutorite(id: number,iddetail: number, payload: any): Observable<any> {
    const headers = this.getAuthHeaders();
    return this.http.post<any>(`${this.apiUrl}/retourAutorite/${id}/${iddetail}`, payload, { headers })
      .pipe(catchError(this.handleError));
  }
  envoiReclamant(reclamationId: number, payload: any): Observable<any> {
    const headers = this.getAuthHeaders();
    return this.http.post<any>(`${this.apiUrl}/envoiReclamant/${reclamationId}`, payload, { headers })
      .pipe(catchError(this.handleError));
  }
  ...
  searchOuverture(numInscription: string, referenceBo: string): Observable<any[]> {
    const headers = this.getAuthHeaders();
    return this.http.get<any[]>(`${this.apiUrl}/ouverture/search`, {
      headers,
      params: { numInscription, referenceBo }
    }).pipe(catchError(this.handleError));
  }
  
  // Recherche pour clôture : réclamations ouvertes (flagCloture IS NULL et flagEnvoiReclamant = 1)
  searchCloture(numInscription: string, referenceBo: string): Observable<any[]> {
    const headers = this.getAuthHeaders();
    return this.http.get<any[]>(`${this.apiUrl}/cloture/search`, {
      headers,
      params: { numInscription, referenceBo }
    }).pipe(catchError(this.handleError));
  }
  
  // Update pour ouverture (réouverture) : met flagCloture à NULL
  ouvertureReclamation(reclamationId: number): Observable<any> {
    const headers = this.getAuthHeaders();
    return this.http.post(`${this.apiUrl}/ouverture/${reclamationId}`, {}, { headers })
      .pipe(catchError(this.handleError));
  }
  
  // Update pour clôture : met flagCloture à 1
  clotureReclamation(reclamationId: number): Observable<any> {
    const headers = this.getAuthHeaders();
    return this.http.post(`${this.apiUrl}/cloture/${reclamationId}`, {}, { headers })
      .pipe(catchError(this.handleError));
  }
